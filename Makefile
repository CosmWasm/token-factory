...
format: format-tools
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs goimports -w -local github.com/CosmWasm/token-factory


###############################################################################
###                                Protobuf                                 ###
###############################################################################
protoImageName=tendermintdev/sdk-proto-gen:v0.7
...
proto-lint:
	@$(DOCKER_BUF) lint --error-format=json proto/
